...
func Setup(app *fiber.App, chatServer *controllers.ChatServer, closeWsChan chan string, protectedUids map[primitive.ObjectID]struct{}, protectedRids map[primitive.ObjectID]struct{}) {
	...
	app.Post("/api/user/updatepfp", helpers.AuthMiddleware, controllers.HandleUpdatePfp(chatServer, protectedUids))
	...
	app.Patch("/api/room/:id", helpers.AuthMiddleware, controllers.HandleUpdateRoom(protectedRids))
	app.Delete("/api/room/:id", helpers.AuthMiddleware, controllers.HandleDeleteRoom(chatServer, protectedRids))
